...
class JWTScanner:
    ...
    def _analyze_payload(self, token: str, url: str) -> list[Vulnerability]:
        ...
        if found_sensitive:
            vulns.append(Vulnerability(
                ...
                description=(
                    f"JWT payload contains sensitive data: {found_sensitive}. "
                    f"JWT can be base64 decoded — it is not encrypted."
                ),
                ...
                remediation=(
                    ...
                    "2. Keep only minimal data like user ID\n"
                    "3. Use JWE (encrypted JWT) if necessary"
                ),
                cwe_id="CWE-522",
            ))
        ...
        if "exp" not in payload:
            vulns.append(Vulnerability(
                ...
                description="Token expiration not set. Token remains valid indefinitely.",
                evidence=f"Payload: {json.dumps(payload)[:200]}",
                exploitation="Stolen token never expires.",
                remediation="Add 'exp' claim to JWT. Recommended: max 1 hour.",
                cwe_id="CWE-613",
            ))
        ...
        if header.get("alg") == "RS256":
            vulns.append(Vulnerability(
                ...
                severity=Severity.HIGH,
                cvss_score=8.1,
                title="Potential JWT Algorithm Confusion (RS256→HS256)",
                description=(
                    "Server uses RS256. If the server accepts the public key as an HS256 "
                    "secret, the token can be forged."
                ),
                evidence=f"alg: {header.get('alg')}",
                exploitation=(
                    "1. Get the server's public key\n"
                    "2. Change header to HS256\n"
                    "3. Sign using the public key as HMAC secret:\n"
                    "   python3 -c \"\n"
                    "   import jwt, requests\n"
                ...
                remediation=(
                    "1. Apply algorithm whitelist on server — accept only RS256\n"
                    "2. Reject algorithm changes\n"
                    "3. In PyJWT use the algorithms=['RS256'] parameter"
                ),
                cwe_id="CWE-327",
    ...
    async def scan(self, url: str) -> list[Vulnerability]:
        console.print(f"  [dim]JWT scan: {url[:60]}[/dim]")
        vulns = []
        ...
        for token in tokens:
            ...
            if found_secret is not None:
                vulns.append(Vulnerability(
                    ...
                    cvss_score=9.8,
                    title=f"JWT Weak Secret Found: '{found_secret}'",
                    description=(
                        f"JWT HMAC secret was found via brute force: '{found_secret}'. "
                        f"Any token can be forged."
                    ),
                    evidence=f"Signature matched with secret: '{found_secret}'",
                    exploitation=(
                        f"python3 -c \"\n"
                        f"import jwt\n"
                    ...
                    remediation=(
                        "1. Change the secret immediately — use a minimum 256-bit random key\n"
                        "2. Invalidate all existing tokens\n"
                        "3. Generate a new secret using: python secrets.token_hex(32)\n"
                        "4. Switch to RSA/ECDSA (RS256/ES256)"
                    ),
            ...
            if forged:
                vulns.append(Vulnerability(
                    ...
                    description=(
                        "Server may accept 'alg:none' tokens. "
                        "It's possible to send an arbitrary payload without a signature."
                    ),
                    evidence="Forged token created with alg:none",
                    exploitation=(
                        f"Write this token in the Authorization header:\n"
                        f"Authorization: Bearer ***"
                        f"Curl:\ncurl -H 'Authorization: Bearer ***' {url}"
                    ),
                    remediation=(
                        "1. Reject the 'none' algorithm in the JWT library\n"
                        "2. Apply an algorithm whitelist\n"
