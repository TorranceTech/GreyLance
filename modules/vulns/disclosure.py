...
class DisclosureScanner:
        ...
        for name, (pattern, severity, cvss) in SENSITIVE_PATTERNS.items():
            ...
            if match:
                ...
                vulns.append(Vulnerability(
                    ...
                    description=f"{name} pattern detected in response.",
                    evidence=f"Pattern found: {redacted}",
                    exploitation=(
    ...
    async def _check_path(self, base_url: str, path: str) -> list[Vulnerability]:
        ...
        if "Index of /" in content or "Directory listing" in content.lower():
            vulns.append(Vulnerability(
                ...
                description="Web server has directory listing enabled. File structure is visible.",
                evidence=f"Received directory listing response for '{path}'",
                exploitation=f"curl {url} — list all files",
        ...
        if ".git" in path and "ref:" in content:
            vulns.append(Vulnerability(
                ...
                description="Git repository is publicly accessible. Source code, credentials, and history can be stolen.",
                evidence=f"HEAD file read: {content[:100]}",
                exploitation=(
                    "Use git-dumper to clone the entire repo:\n"
        ...
        # Sensitive file is just accessible
            ...
            for sensitive_ext in [".env", ".sql", ".key", ".pem", "phpinfo"]:
                if sensitive_ext in path.lower():
                    vulns.append(Vulnerability(
                        ...
                        description=f"'{path}' file is publicly accessible.",
                        evidence=f"HTTP 200 response, content length: {len(content)}",
                        exploitation=f'curl "{url}" > stolen_{path.replace("/", "_")}',
                        remediation="Remove this file from web root or hide it on the web server.",
                        cwe_id="CWE-538",
                    ))
                    break
    ...
    async def scan(self, base_url: str) -> list[Vulnerability]:
        console.print(f"  [dim]Info disclosure scan: {len(SENSITIVE_PATHS)} paths...[/dim]")
